...
async function runFailurePath(): Promise<void> {
	...
	try {
		...
		assert(message.includes("Process exited before response"), `expected process-exited detail, got ${message}`);
		assert(rpc.status === "error", `expected teammate status to be error after handshake failure, got ${rpc.status}`);
		assert(
			typeof rpc.lastError === "string" && rpc.lastError.includes("Process exited before response"),
